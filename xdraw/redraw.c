...
-----------------------------------------------------------------------------*/
int redraw0(int ic, int ipair, int npair,
	     float xmin, float xmax, float ymin, float ymax)
{
  ...
  for(kk=0; kk<nf; kk++)		/*..........Loop through family */
    {
      //printf("View %d, Draw family member %d of %d curveset %d, ipair=%d, labelf=%d\n", 
      //iview, kk, nf, ic, ipair, flags & LABELF);
	...
	for(jcurve=drawMe=0; jcurve<9; jcurve++) {
	  if ( npair>1 && cp->icurve[jcurve] == ipair) drawMe=1;
	  else if (npair==1 && cp->icurve[jcurve] == kk) drawMe=1;
	}
	if (!drawMe) continue;
      }
	...
	setcolor(k, nf, -1);
      ...
      /* NN 
      x1 = float_to_int(fx1old);
      y1 = float_to_int(fy1old);
      NN */
...
|	* doesn't draw, just scans curves and eg gets limits
-----------------------------------------------------------------------------*/
...
#ifdef DEAD_CODE
	  if (k==0) {
	    printf("Redraw00 in SINGLE mode, curveset=%d,  k=%d:\n", ic, k);
	    for(i=0; i<9; i++) printf(" %d. %d ", i, cp->icurve[i]);
	    printf("\n");
	  }
#endif
	  for(i=0, skipMe=1; i<9; i++) if (cp->icurve[i] == (k+ipair)) skipMe = 0;
	  if (debug_scale_evis) printf("redraw00, ic=%d, skipMe=%d\n", ic, skipMe);
	  //if (skipMe) printf("k=%d+%d, SkipMe!\n", k, ipair);
	  //else printf("k=%d+%d, Go!\n", k, ipair);
	  if (skipMe) continue;
	}
      if (k%cp->fskip !=0) continue;
