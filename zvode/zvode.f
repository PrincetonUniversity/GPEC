...
C     /ZVOD02/ --  HU
C
C Subroutines called by ZVINDY: DZSCAL, XERRWD3
...
*DECK DUMACH3
      FUNCTION DUMACH3 ()
C***BEGIN PROLOGUE  DUMACH3
C***PURPOSE  Compute the unit roundoff of the machine.
C***CATEGORY  R1
C***TYPE      DOUBLE PRECISION (RUMACH-S, DUMACH3-D)
C***KEYWORDS  MACHINE CONSTANTS
C***AUTHOR  Hindmarsh, Alan C., (LLNL)
C***DESCRIPTION
C *Usage:
C        DOUBLE PRECISION  A, DUMACH3
C        A = DUMACH3()
C
...
C     number u such that  1.0 + u .ne. 1.0.  This is computed by DUMACH3
...
C***END PROLOGUE  DUMACH3
C
      DOUBLE PRECISION DUMACH3
      DOUBLE PRECISION U, COMP
C***FIRST EXECUTABLE STATEMENT  DUMACH3
      U = 1.0D0
 10   U = U*0.5D0
      ...
      DUMACH3 = U*2.0D0
      RETURN
C----------------------- End of Function DUMACH3 ------------------------
      END
      SUBROUTINE DUMSUM(A,B,C)
C     Routine to force normal storing of A + B, for DUMACH3.
      DOUBLE PRECISION A, B, C
      C = A + B
      RETURN
      END
*DECK XERRWD3
      SUBROUTINE XERRWD3(MSG, NMES, NERR, LEVEL, NI, I1, I2, NR, R1, R2)
C***BEGIN PROLOGUE  XERRWD3
C***SUBSIDIARY
C***PURPOSE  Write error message with values.
C***CATEGORY  R3C
C***TYPE      DOUBLE PRECISION (XERRWV-S, XERRWD3-D)
C***AUTHOR  Hindmarsh, Alan C., (LLNL)
C***DESCRIPTION
C
C  Subroutines XERRWD3, XSETF, XSETUN, and the function routine IXSAV3,
...
C     in D21.13 format.
C
C***ROUTINES CALLED  IXSAV3
C***REVISION HISTORY  (YYMMDD)
C   920831  DATE WRITTEN
C   921118  Replaced MFLGSV/LUNSAV by IXSAV3. (ACH)
...
C***END PROLOGUE  XERRWD3
C
C*Internal Notes:
C
C For a different default logical unit number, IXSAV3 (or a subsidiary
C routine that it calls) will need to be modified.
C For a different run-abort command, change the statement following
C statement 100 at the end.
C-----------------------------------------------------------------------
C Subroutines called by XERRWD3.. None
C Function routine called by XERRWD3.. IXSAV3
C-----------------------------------------------------------------------
C**End
C
C  Declare arguments.
...
C
      INTEGER LUNIT, IXSAV3, MESFLG
C
C  Get logical unit number and message print flag.
C
C***FIRST EXECUTABLE STATEMENT  XERRWD3
      LUNIT = IXSAV3 (1, 0, .FALSE.)
      MESFLG = IXSAV3 (2, 0, .FALSE.)
...
C Function routine called by XSETF.. IXSAV3
C-----------------------------------------------------------------------
C**End
      INTEGER MFLAG, JUNK, IXSAV3
C
C***FIRST EXECUTABLE STATEMENT  XSETF
      IF (MFLAG .EQ. 0 .OR. MFLAG .EQ. 1) JUNK = IXSAV3 (2,MFLAG,.TRUE.)
      RETURN
...
C Function routine called by XSETUN.. IXSAV3
C-----------------------------------------------------------------------
C**End
      INTEGER LUN, JUNK, IXSAV3
C
C***FIRST EXECUTABLE STATEMENT  XSETUN
      IF (LUN .GT. 0) JUNK = IXSAV3 (1,LUN,.TRUE.)
      RETURN
C----------------------- End of Subroutine XSETUN ----------------------
      END
*DECK IXSAV3
      FUNCTION IXSAV3 (IPAR, IVALUE, ISET)
C***BEGIN PROLOGUE  IXSAV3
C***SUBSIDIARY
...
C***DESCRIPTION
C
C  IXSAV3 saves and recalls one of two error message parameters:
...
C            by a call to IUMACH3 (may be machine-dependent).
...
C
C  On return..
C    IXSAV3 = The (old) value of the parameter.
C
C***SEE ALSO  XERRWD3, XERRWV
C***ROUTINES CALLED  IUMACH3
C***REVISION HISTORY  (YYMMDD)
C   921118  DATE WRITTEN
C   930329  Modified prologue to SLATEC format. (FNF)
C   930915  Added IUMACH3 call to get default output unit.  (ACH)
C   930922  Minor cosmetic changes. (FNF)
C   010425  Type declaration for IUMACH3 added. (ACH)
C***END PROLOGUE  IXSAV3
C
C Subroutines called by IXSAV3.. None
C Function routine called by IXSAV3.. IUMACH3
C-----------------------------------------------------------------------
C**End
      INTEGER IXSAV3
      LOGICAL ISET
...
C
C***FIRST EXECUTABLE STATEMENT  IXSAV3
      IF (IPAR .EQ. 1) THEN
        IF (LUNIT .EQ. -1) LUNIT = IUMACH3()
        IXSAV3 = LUNIT
...
C
      RETURN
C----------------------- End of Function IXSAV3 -------------------------
      END
*DECK IUMACH3
      FUNCTION IUMACH3()
C***BEGIN PROLOGUE  IUMACH3
C***PURPOSE  Provide standard output unit number.
C***CATEGORY  R1
C***TYPE      INTEGER (IUMACH3-I)
C***KEYWORDS  MACHINE CONSTANTS
C***AUTHOR  Hindmarsh, Alan C., (LLNL)
C***DESCRIPTION
C *Usage:
C        INTEGER  LOUT, IUMACH3
C        LOUT = IUMACH3()
C
C *Function Return Values:
...
c
         ...
         do 80 kb = 1, nm1
            k = n - kb
            b(k) = b(k) + zdotc3(n-k,a(k+1,k),1,b(k+1),1)
            l = ipvt(k)
            if (l .eq. k) go to 70
               t = b(l)
               b(l) = b(k)
...
c
         ...
         if (nm1 .lt. 1) go to 90
            do 80 kb = 1, nm1
               k = n - kb
               lm = min0(ml,n-k)
               b(k) = b(k) + zdotc3(lm,abd(m+1,k),1,b(k+1),1)
               l = ipvt(k)
               if (l .eq. k) go to 70
                  t = b(l)
                  b(l) = b(k)
...
*DECK ZDOTC3
      double complex function zdotc3(n,zx,incx,zy,incy)
c
