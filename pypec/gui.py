...
class InputPanel(HasTraits):
    ...
    def trait_view(self, name=None, view_elements=None):
        return View(
            Group(*self.items, layout=self.layout, show_labels=self.layout == "normal"),
            resizable=True,
            height=600,
            buttons=NoButtons,
            scrollable=True,
...
class ControlPanel(HasTraits):
    ...
    view = View(
        Item("rundir", label="Run Directory", width=300),
        Item("location", label="Location", width=300),
        Group(
            Item("mailon", label="Mail On"),
            Item("email", label="email", width=200, visible_when="mailon!=''"),
            orientation="horizontal",
        ),
        Group(
            Item("load_all", show_label=False),
            Item("load_dir", show_label=False, width=300),
            columns=2,
            label="New Inputs from Directory",
            show_border=True,
        ),
        Group(
            UItem("load", show_label=False),
            Item("load_file", show_label=False, width=300),
            columns=2,
            label="New NameList from File",
            show_border=True,
        ),
        Group(
            Item("rundcon", label="DCON"),
            Item("rungpec", label="GPEC"),
            Item("runpent", label="PENT"),
            orientation="horizontal",
        ),
        Group(
            Item("qsub", label="Submit to Cluster"),
    ...
    def _run_fired(self):
        ...
        gpec.run(
            ...
            qsub=self.qsub,
            return_on_complete=self.roc,
            mailon=str(self.mailon),
            email=str(self.email),
            rundcon=self.rundcon,
            rungpec=self.rungpec,
            runpent=self.runpent,
