...
class LinearBase:
        ...
        **kwargs   : dict. Passed to Data.interp
        ...
        for k in data.y.keys():
            if ("angle " in k and k.replace("angle ", "") in keys) or (
                ...
                keys.remove(k)
        if not quiet:
            print("Interpolating {:} to {:}".format(keys, self.xnames[:2]))
    ...
    def mean(self, datakeys=None):
        ...
        # housekeeping
        if not datakeys:
            datakeys = np.sort(self.data.keys()).tolist()
        if not type(datakeys) in (list, tuple):
            datakeys = (datakeys,)
        # calculation
        datamean = {}
        for k in datakeys:
...
class RZCrossSection(LinearBase):
    def __init__(
        self, r, z, mtheta=540, r0=1.69, z0=0, t0=-np.pi / 2, close=False, name=""
    ):
        """
        ...
        self.pts["theta"] = (
            np.arctan2(self.pts["z"] - z0, self.pts["r"] - r0) - t0
        ) % (2 * np.pi)
    ...
    def set_rzcharacteristics(self):
        ...
        for k in keys:
            self.data[k + "_prp"] = -1 * (
                self.data[k + "_r"] * sina - self.data[k + "_z"] * cosa
            )
            self.data[k + "_par"] = 1 * (
                self.data[k + "_r"] * cosa + self.data[k + "_z"] * sina
            )
            for suffix in ["_prp", "_par"]:
                if np.any(np.iscomplex(self.data[k + suffix])):
                    self.data["angle " + k + suffix] = np.angle(
                        self.data[k + suffix]
                    )  # phase
                    self.data["| " + k + suffix + " |"] = np.abs(
                        self.data[k + suffix]
                    )  # amplitude
        ...
        **kwargs : dict. Valid pcolormesh/contour/contourf key arguments.
        ...
        if "figure" in kwargs:
            ...
            f, ax = plt.subplots(
                ...
                figsize=plt.rcp_size * (ncol, nrow),
            )  # (4+2*ncol,3+2*nrow))
        ax = np.array(ax)
        for kwarg in ["figure", "axes"]:
            if kwarg in kwargs:
...
class AxisymSurface(SurfaceBase):
    ...
    def __init__(self, RZCrossSection, phi_extent=(0, 2 * np.pi), nphi=180):
        ...
        **kwargs  : dict. Passed to RZCrossSection.
        ...
        self.pts["theta"] = (
            self.CrossSection.pts["theta"].repeat(nphi).reshape(-1, nphi)
        )
        self.pts["theta (deg)"] = self.pts["theta"] * 180 / np.pi - 90
        phi = np.linspace(phi_extent[0], phi_extent[1], nphi)
        self.pts["phi"] = np.array([phi]).repeat(
            len(self.CrossSection.pts["theta"]), axis=0
        )
    ...
    def set_data(self, data, quiet=True, **kwargs):
        ...
        **kwargs - dict. Passed to Data.interp
        ...
        for k in keys:
            self.data[k] = np.real(
                self.CrossSection.data[k].reshape(-1, 1)
...
_d3d_nodes = {
    ...
    "DIII-D Center Stack": [
        np.array([95.748, 95.748, 95.748, 95.748]) / 100,
        np.array([-126.083, -1e-10, 1e-10, 126.083]) / 100,
    ],
    "DIII-D Low Field Side": [
        np.array([168.285, 216.931, 242.824, 242.824, 216.931, 183.657]) / 100,
        np.array([142.545, 103.195, 40.310, -39.916, -102.799, -142.342]) / 100,
    ],
    "DIII-D Lower Baffle": [
        np.array([2.134, 1.786, 1.768, 1.768, 1.682, 1.372]),
        np.array([-0.9730, -1.174, -1.211, -1.250, -1.250, -1.250]),
    ],
    "DIII-D Upper Baffle": [
        np.array([1.372, 1.608, 1.647, 1.785, 2.070, 2.128]),
...
for k, v in _d3d_nodes.items():
    close = "Vessel" in k
    d3d_crosssections[k] = RZCrossSection(
        v[0], v[1], r0=1.69, z0=0, name=k, close=close
    )
    ...
    *Key Word Arguments:*
        ...
        for i in range(500):
            for j in range(500):
                if "coil_cur({:},{:})".format(i + 1, j + 1) in coil["COIL_CONTROL"]:
                    curs.append(
                        coil["COIL_CONTROL"]["coil_cur({:},{:})".format(i + 1, j + 1)]
                    )
    ...
    for i in range(1, 50):
        if "coil_name({:})".format(i) in coil["COIL_CONTROL"]:
            ...
            x, y, z = np.genfromtxt(
                packagedir + "coil/" + cfile, skip_header=1
            ).T.reshape(3, ncoil, s, nsec)
            cur = [
                coil["COIL_CONTROL"]["coil_cur({:},{:})".format(i, j + 1)]
            ...
            if machine in r0:
                ...
                print(
                    "WARNING: {:} major radius unknown... no 2D plots available.".format(
                        machine
                    )
                )
            coils[name]["cur"] = cur
            ...
            for j in range(ncoil):
                if curlim:
                    kwargs["color"] = current_rgb(cur[j])
                if dim == 3:
                    data.mmlab.plot3d(
                        x[j].ravel(),
                        y[j].ravel(),
                        z[j].ravel(),  # cur[j]*np.ones_like(z[j].ravel()),
                        name=name + str(j),
                        **kwargs
                    )
                elif dim == 2:
                    axes.plot(
                        coils[name]["phi"][j].ravel() / dtor,
                        coils[name]["theta"][j].ravel() / dtor,
                        label=name + str(j),
                        **kwargs
                    )
                else:
                    axes.plot(
                        coils[name]["r"][j].ravel(),
                        coils[name]["z"][j].ravel(),
                        label=name + str(j),
                        **kwargs
                    )
...
def _coil_to_netcdf(name, machine):
    ...
    *Arguments:*
    ...
    with open(packagedir + "coil/" + cfile, "r") as f:
        line1 = f.readline()
        ncoil, s, nsec, nw = map(int, map(float, line1.split()))
    x, y, z = np.genfromtxt(packagedir + "coil/" + cfile, skip_header=1).T.reshape(
        3, ncoil, s, nsec
    )
    cur = [
        coil["COIL_CONTROL"]["coil_cur({:},{:})".format(i, j + 1)] for j in range(ncoil)
    ]
    # store info
    coils[name] = {}
    coils[name]["x"] = x
...
class _magnetics(object):
    ...
    def get_signal(self, key, ds, op="mean"):
        ...
        *Returns:*
        ...
        bz = interpn(
            (ds["R"].values, ds["z"].values), ds["b_z-plas"].values.T, v["rzs"]
        )
        br = interpn(
            (ds["R"].values, ds["z"].values), ds["b_r-plas"].values.T, v["rzs"]
        )
