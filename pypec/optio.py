...
def svd_transform(
    ...
    **Returns:**
    ...
    keys, perpkeys = [], []
    v = np.vstack((np.identity(mpert), np.identity(mpert) * 1j))
    if phi:
        prefix = "Phi"
    else:
        prefix = "b"
    ...
    header = "{:>16} {:>16}\n{:16} {:16}".format(
        "num_vec", "num_m", int(svd.params["msing"]), mpert
    )
    tmp = svd.params
    svd.params = {}
    ...
    if nperp > 0:
        ...
        header = "{:>16} {:>16}\n{:16} {:16}".format(
            "num_vec", "num_m", 2 * mpert - nperp, mpert
        )
...
def eigenvectors(opt_file, ms=range(-30, 30), ortho=None, phi=False, **kwargs):
    ...
    header = "{:>16} {:>16}\n{:16} {:16}\n\n".format(
        "num_vec", "num_m", cv.shape[0] / 2, cv.shape[1]
    )
    header += "{:>16}".format("m")
    for i in range(1, v.shape[0] / 2 + 1):
        header += " {:>16} {:>16}".format(
            "real(" + prefix + str(i) + ")", "imag(" + prefix + str(i) + ")"
        )
