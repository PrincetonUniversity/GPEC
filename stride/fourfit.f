...
c-----------------------------------------------------------------------
      ...
      INTEGER, DIMENSION(:), ALLOCATABLE :: ipiva
      COMPLEX(r8), DIMENSION(:,:), ALLOCATABLE :: amat,bmat,cmat
      COMPLEX(r8), DIMENSION(:), ALLOCATABLE :: jmat
      LOGICAL :: fourfit_metric_parallel
...
c-----------------------------------------------------------------------
      ...
      INTEGER :: rz_ix,rz_iy
      REAL(r8), DIMENSION(:), ALLOCATABLE :: rzphi_s_f,rzphi_s_fx,
     $     rzphi_s_fy
c-----------------------------------------------------------------------
...
!$OMP& PRIVATE(ipsi,psifac,itheta,theta,rfac,eta,r,jac,jac1,v)
!$OMP& PRIVATE(rzphi_s_f,rzphi_s_fx,rzphi_s_fy)
!$OMP& FIRSTPRIVATE(rz_ix,rz_iy)
!$OMP& SHARED(mpsi,mtheta,rzphi,metric,ro,sq)
      ALLOCATE(rzphi_s_f(SIZE(rzphi%f)),rzphi_s_fx(SIZE(rzphi%fx)),
     $     rzphi_s_fy(SIZE(rzphi%fy)))
!$OMP DO SCHEDULE(STATIC,8)
     ...
     $           rzphi_s_fy)
            ...
            rfac=SQRT(rzphi_s_f(1))
            eta=twopi*(theta+rzphi_s_f(2))
            r=ro+rfac*COS(eta)
            jac=rzphi_s_f(4)
            jac1=rzphi_s_fx(4)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
            v(1,1)=rzphi_s_fx(1)/(2*rfac*jac)
            v(1,2)=rzphi_s_fx(2)*twopi*rfac/jac
            v(1,3)=rzphi_s_fx(3)*r/jac
            v(2,1)=rzphi_s_fy(1)/(2*rfac*jac)
            v(2,2)=(1+rzphi_s_fy(2))*twopi*rfac/jac
            v(2,3)=rzphi_s_fy(3)*r/jac
            v(3,3)=twopi*r/jac
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     $     fmat,gmat,hmat,kmat,temp1,temp2,tM
...
!$OMP& tM,temp1,temp2,amat,bmat,cmat,dmat,emat,hmat,fmat,kmat,gmat,
!$OMP& fmatb,iqty,message)
!$OMP& SHARED(mlow,mhigh,nn,imat,mpsi,psio,metric,mpert,mband,sq)
!$OMP& SHARED(sas_flag,amats,bmats,cmats,feval_flag)
!$OMP& LASTPRIVATE(jmat)
!$OMP& SHARED(fmats,gmats,kmats)
!$OMP& SCHEDULE(GUIDED)
      DO ipsi=0,mpsi
         psifac=sq%xs(ipsi)
         p1=sq%fs1(ipsi,2)
...
c-----------------------------------------------------------------------
         ...
         CALL ZGEMM('C','N',mpert,mpert,mpert,negone_c,dmat,mpert,temp1,
     $        mpert,one_c,fmat,mpert)
         tM = kmat
         kmat = emat
         CALL ZGEMM('C','N',mpert,mpert,mpert,negone_c,tM,mpert,temp2,
     $        mpert,one_c,kmat,mpert)
         gmat = hmat
         CALL ZGEMM('C','N',mpert,mpert,mpert,negone_c,cmat,mpert,temp2,
     $        mpert,one_c,gmat,mpert)
c-----------------------------------------------------------------------
c     diagnose.
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      IF(power_flag)THEN
         ...
         DO jpert=1,mpert
            DO ipert=MAX(1,jpert-mband),MIN(mpert,jpert+mband)
               dm=ipert-jpert
               IF(m == 1 .AND. dm == -1 .OR. m == -1 .AND. dm == 1)THEN
...
c-----------------------------------------------------------------------
      CALL cspline_fit(fmats,spline_str)
      CALL cspline_fit(kmats,spline_str)
      CALL cspline_fit(gmats,spline_str)
c-----------------------------------------------------------------------
c     interpolate matrices to psilim.
c-----------------------------------------------------------------------
      IF(sas_flag)THEN
         CALL cspline_fit(amats,spline_str)
         CALL cspline_fit(bmats,spline_str)
         CALL cspline_fit(cmats,spline_str)
