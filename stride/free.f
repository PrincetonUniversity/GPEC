...
c-----------------------------------------------------------------------
      ...
      COMPLEX(r8), DIMENSION(:,:), ALLOCATABLE :: wp,wv

      CONTAINS
c-----------------------------------------------------------------------
c     subprogram 1. free_calc_wp.
c     computes the plasma response matrix.
c-----------------------------------------------------------------------
c-----------------------------------------------------------------------
c     declarations.
c-----------------------------------------------------------------------
      SUBROUTINE free_calc_wp()

      INTEGER :: ipert, sTime, fTime, cr
      COMPLEX(r8), DIMENSION(mpert,mpert) :: temp,tempUIn,tempU,tempVt
      REAL(r8), DIMENSION(mpert) :: tempS
      COMPLEX(r8), DIMENSION(3*mpert) :: tempWork
      REAL(r8), DIMENSION(5*mpert) :: tempRWork
      INTEGER, DIMENSION(mpert) :: ipiv
      INTEGER :: info, tempInfo, temp3M
c-----------------------------------------------------------------------
c     initialize.
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      ENDIF
c-----------------------------------------------------------------------
c     print singular values of wp (scaled by psio^2).
c-----------------------------------------------------------------------
      IF (verbose_performance_output) THEN
         tempUIn = wp
         temp3M = 3*mpert
         CALL zgesvd('A','A',mpert,mpert,tempUIn,mpert,tempS,tempU,
     $        mpert,tempVt,mpert,tempWork,temp3M,tempRWork,tempInfo)
         DO ipert=1,mpert
            WRITE(*,'(es13.5)')tempS(ipert)*psio**2
         ENDDO
      ENDIF
c-----------------------------------------------------------------------
c     print timer.
...
c-----------------------------------------------------------------------
      RETURN
      END SUBROUTINE free_calc_wp
c-----------------------------------------------------------------------
c     subprogram 2. free_calc_wv.
c     computes the vacuum response matrix.
c-----------------------------------------------------------------------
c-----------------------------------------------------------------------
c     declarations.
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      CALL free_write_msc
      IF(ahb_flag)THEN
         CALL program_stop("wp is ill defined, so ahb is disabled.")
...
c-----------------------------------------------------------------------
c     calculate v1, the psilim point.
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     $     "mthsurf = ",mthsurf,", mpert = ",mpert,", nn = ",nn,
     $     ", qsurf = ",qsurf
