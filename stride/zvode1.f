C---------------------------------------------------------------------
C     NOTES: (1) Assigned RPAR as DOUBLE COMPLEX in all cases below.
C                Could have chosen REAL, DOUBLE REAL, or COMPLEX.
C            (2) Commented out "EXTERNAL ZVNLSD1" b/c now internal.
C            (3) Renamed all subroutines and common blocks with a
C                "1" after the name.
C---------------------------------------------------------------------
      MODULE zvode1_mod
      ...
      DOUBLE PRECISION DBL_ZVODE11, DBL_ZVODE21
      INTEGER INT_ZVODE11, INT_ZVODE21
      COMMON /ZVOD011/ DBL_ZVODE11(50), INT_ZVODE11(33)
      COMMON /ZVOD021/ DBL_ZVODE21(9), INT_ZVODE21(8)

!$OMP THREADPRIVATE(/ZVOD011/,/ZVOD021/)

        CONTAINS

        SUBROUTINE ZVODE1 (F, NEQ, Y, T, TOUT, ITOL, RTOL, ATOL, ITASK,
     1              ISTATE, IOPT, ZWORK, LZW, RWORK, LRW, IWORK, LIW,
     2              JAC, MF, RPAR, IPAR)
        EXTERNAL F, JAC
        DOUBLE COMPLEX Y, ZWORK, RPAR
...
C-----------------------------------------------------------------------
C ZVODE1: Variable-coefficient Ordinary Differential Equation solver,
C with fixed-leading-coefficient implementation.
C This version is in complex double precision.
C
C ZVODE1 solves the initial value problem for stiff or nonstiff
C systems of first order ODEs,
C     dy/dt = f(t,y) ,  or, in component form,
C     dy(i)/dt = f(i) = f(i,t,y(1),y(2),...,y(NEQ)) (i = 1,...,NEQ).
C Here the y vector is treated as complex.
C ZVODE1 is a package based on the EPISODE and EPISODEB packages, and
C on the ODEPACK user interface standard, with minor modifications.
C
C NOTE: When using ZVODE1 for a stiff system, it should only be used for
...
C fact is critical in the way ZVODE1 solves the dense or banded linear
C systems that arise in the stiff case.  For a complex stiff ODE system
C in which f is not analytic, ZVODE1 is likely to have convergence
...
C Summary of usage.
C
C Communication between the user and the ZVODE1 package, for normal
...
C choices for MF (21, 22, 24, 25), and ZVODE1 requires the Jacobian
C matrix in some form.  In these cases (MF .gt. 0), ZVODE1 will use a
...
C or banded (MF = 24 or 25).  In the banded case, ZVODE1 requires two
...
C directly (MF = 21 or 24), but if this is not feasible, ZVODE1 will
...
C
C D. Write a main program which calls subroutine ZVODE1 once for
C each point at which answers are desired.  This should also provide
C for possible use of logical unit 6 for output of error messages
C by ZVODE1.  On the first call to ZVODE1, supply arguments as follows:
...
C ISTATE = 2  if ZVODE1 was successful, negative otherwise.
...
C reset TOUT and call ZVODE1 again.  No other parameters need be reset.
C
C-----------------------------------------------------------------------
C EXAMPLE PROBLEM
C
C The program below uses ZVODE1 to solve the following system of 2 ODEs:
...
C     DO 40 IOUT = 1,40
C       CALL ZVODE1(FEX,NEQ,Y,T,TOUT,ITOL,RTOL,ATOL,ITASK,ISTATE,IOPT,
...
C
C-----------------------------------------------------------------------
C Full description of user interface to ZVODE1.
C
C The user interface to ZVODE1 consists of the following parts.
C
C i.   The call sequence to subroutine ZVODE1, which is a driver
...
C
C ii.  Descriptions of other routines in the ZVODE1 package that may be
...
C
C iv.  Description of two routines in the ZVODE1 package, either of
...
C to the return from subroutine ZVODE1 to the user's calling program.)
C
...
C          externally to ZVODE1, an extra call to F should be made
C          for this purpose, for consistent and accurate results.
C          If only the derivative dy/dt is needed, use ZVINDY1 instead.
C
...
C          (This will be checked by the solver.)
C
C Note:  The work arrays must not be altered between calls to ZVODE1
...
C scratch space, and so is available for use by the user outside ZVODE1
...
C          or loaded arbitrarily, as they are overwritten by ZVODE1.
...
C          RPAR is not type-declared within ZVODE1, but simply passed
...
C Optional Output.
C
C As optional additional output from ZVODE1, the variables listed
C below are quantities related to the performance of ZVODE1
C which are available to the user.  These are communicated by way of
C the work arrays, but also have internal mnemonic names as shown.
C Except where stated otherwise, all of this output is defined
C on any successful return from ZVODE1, and on any return with
...
C                        defined only on a successful return from ZVODE1.
C
C-----------------------------------------------------------------------
C Interrupting and Restarting
C
C If the integration of a given problem by ZVODE1 is to be
...
C the user should save, following the return from the last ZVODE1 call
...
C values before the next ZVODE1 call for that problem.  To save
C and restore the COMMON blocks, use subroutine ZVSRCO1, as
...
C gain additional capabilities in conjunction with ZVODE1.
C (The routines XSETUN and XSETF are designed to conform to the
C SLATEC error handling package.)
C
C     FORM OF CALL                  FUNCTION
C  CALL XSETUN(LUN)           Set the logical unit number, LUN, for
C                             output of messages from ZVODE1, if
C                             the default is not desired.
C                             The default value of LUN is 6.
C
C  CALL XSETF(MFLAG)          Set a flag to control the printing of
C                             messages by ZVODE1.
...
C
C  CALL ZVSRCO1(RSAV,ISAV,JOB) Saves and restores the contents of
C                             the internal COMMON blocks used by
C                             ZVODE1. (See Part iii below.)
C                             RSAV must be a real array of length 51
C                             or more, and ISAV must be an integer
C                             array of length 40 or more.
C                             JOB=1 means save COMMON into RSAV/ISAV.
C                             JOB=2 means restore COMMON from RSAV/ISAV.
C                                ZVSRCO1 is useful if one is
C                             interrupting a run and restarting
C                             later, or alternating between two or
C                             more problems solved with ZVODE1.
C
C  CALL ZVINDY1(,,,,,)         Provide derivatives of y, of various
C        (See below.)         orders, at a specified point T, if
C                             desired.  It may be called only after
C                             a successful return from ZVODE1.
C
C The detailed instructions for using ZVINDY1 are as follows.
C The form of the call is:
C
C  CALL ZVINDY1 (T, K, ZWORK, NYH, DKY, IFLAG)
C
C The input parameters are:
C
C T         = Value of independent variable where answers are desired
C             (normally the same as the T last returned by ZVODE1).
...
C             by ZVODE1 directly.  Since NQCUR .ge. 1, the first
C             derivative dy/dt is always available with ZVINDY1.
...
C If ZVODE1 is to be used in an overlay situation, the user
C must declare, in the primary overlay, the variables in:
C   (1) the call sequence to ZVODE1,
C   (2) the two internal COMMON blocks
C         /ZVOD011/  of length  83  (50 double precision words
C                         followed by 33 integer words),
C         /ZVOD021/  of length  9  (1 double precision word
C                         followed by 8 integer words),
C
C If ZVODE1 is used on a system in which the contents of internal
...
C
C Below are descriptions of two routines in the ZVODE1 package which
...
C where NEQ, ITOL, RTOL, and ATOL are as in the ZVODE1 call sequence,
...
C ZVNORM routine (See below.), and also used by ZVODE1 in the computation
...
C     COMMON /ZVOD011/ RVOD(50), IVOD(33)
C     COMMON /ZVOD021/ HU, NCFN, NETF, NFE, NJE, NLU, NNI, NQU, NST
C     NQ = IVOD(28)
C     H = RVOD(21)
...
C value of ZVNORM suitable for use in the error control in ZVODE1.
...
C-----------------------------------------------------------------------
C Other Routines in the ZVODE1 Package.
C
C In addition to Subroutine ZVODE1, the ZVODE1 package includes the
C following subroutines and function routines:
C  ZVHIN     computes an approximate step size for the initial step.
C  ZVINDY1    computes an interpolated value of the y vector at t = TOUT.
C  ZVSTEP1    is the core integrator, which does one step of the
C            integration and the associated error control.
C  ZVSET1     sets all method coefficients and test constants.
C  ZVNLSD1    solves the underlying nonlinear system -- the corrector.
C  ZVJAC1     computes and preprocesses the Jacobian matrix J = df/dy
C            and the Newton iteration matrix P = I - (h/l1)*J.
C  ZVSOL1     manages solution of linear system in chord iteration.
C  ZVJUST1    adjusts the history array on a change of order.
...
C  ZVSRCO1    is a user-callable routine to save and restore
...
C  DUMACH3    sets the unit roundoff of the machine.
C  XERRWD3, XSETUN, XSETF, IXSAV3, and IUMACH3 handle the printing of all
C            error messages and warnings.  XERRWD3 is machine-dependent.
C Note: ZVNORM, ZABSSQ, DUMACH3, IXSAV3, and IUMACH3 are function routines.
...
C-----------------------------------------------------------------------
C
C Type declarations for labeled COMMON block ZVOD011 --------------------
C
     ...
     4        NSLP, NYH
C
C Type declarations for labeled COMMON block ZVOD021 --------------------
C
      DOUBLE PRECISION HU
...
C
      DOUBLE PRECISION DUMACH3, ZVNORM
C
      DIMENSION MORD(2)
C-----------------------------------------------------------------------
C The following Fortran-77 declaration is to cause the values of the
C listed (local) variables to be saved between calls to ZVODE1.
C-----------------------------------------------------------------------
      SAVE MORD, MXHNL0, MXSTP0
      SAVE ZERO, ONE, TWO, FOUR, PT2, HUN
!$OMP THREADPRIVATE(MORD,MXHNL0,MXSTP0,ZERO,ONE,TWO,FOUR,PT2,HUN)
C-----------------------------------------------------------------------
C The following internal COMMON blocks contain variables which are
C communicated between subroutines in the ZVODE1 package, or which are
C to be saved between calls to ZVODE1.
C In each block, real variables precede integers.
C The block /ZVOD011/ appears in subroutines ZVODE1, ZVINDY1, ZVSTEP1,
C ZVSET1, ZVNLSD1, ZVJAC1, ZVSOL1, ZVJUST1 and ZVSRCO1.
C The block /ZVOD021/ appears in subroutines ZVODE1, ZVINDY1, ZVSTEP1,
C ZVNLSD1, ZVJAC1, and ZVSRCO1.
C
...
C DRC    = Relative change in H*RL1 since last ZVJAC1 call.
C EL     = Real array of integration coefficients.  See ZVSET1.
...
C RC     = Ratio of current H*RL1 to value on last ZVJAC1 call.
...
C TQ     = A real vector of length 5 in which ZVSET1 stores constants
...
C ICF    = Integer flag for convergence failure in ZVNLSD1:
...
C JCUR   = Output flag from ZVJAC1 showing Jacobian status:
C            JCUR = 0 means J is not current.
C            JCUR = 1 means J is current.
C JSTART = Integer flag used as input to ZVSTEP1:
...
C          On return, ZVSTEP1 sets JSTART = 1.
C JSV    = Integer flag for Jacobian saving, = sign(MF).
C KFLAG  = A completion code from ZVSTEP1 with the following meanings:
C               0      the step was succesful.
C              -1      the requested error could not be achieved.
C              -2      corrector convergence could not be achieved.
C              -3, -4  fatal error in VNLS (can not occur here).
C KUTH   = Input flag to ZVSTEP1 showing whether H was reduced by the
...
C-----------------------------------------------------------------------
      COMMON /ZVOD011/ ACNRM, CCMXJ, CONP, CRATE, DRC, EL(13), ETA,
     ...
     7                NSLP, NYH
      COMMON /ZVOD021/ HU, NCFN, NETF, NFE, NJE, NLU, NNI, NQU, NST
...
C If ISTATE = 3, set flag to signal parameter changes to ZVSTEP1. -------
      JSTART = -1
...
C-----------------------------------------------------------------------
 ...
 210  IF ((TN - TOUT)*H .LT. ZERO) GO TO 250
      CALL ZVINDY1 (TOUT, 0, ZWORK(LYH), NYH, Y, IFLAG)
...
C the call to the one-step core integrator ZVSTEP1.
C
...
C-----------------------------------------------------------------------
 ...
 280  IF ((TN + H) .NE. TN) GO TO 290
      ...
      MSG = 'ZVODE1--  Warning: internal T (=R1) and H (=R2) are'
      CALL XERRWD3 (MSG, 50, 101, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG='      such that in the machine, T + H = T on the next step  '
      CALL XERRWD3 (MSG, 60, 101, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG = '      (H = step size). solver will continue anyway'
      CALL XERRWD3 (MSG, 50, 101, 1, 0, 0, 0, 2, TN, H)
      IF (NHNIL .LT. MXHNIL) GO TO 290
      MSG = 'ZVODE1--  Above warning has been issued I1 times.  '
      CALL XERRWD3 (MSG, 50, 102, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG = '      it will not be issued again for this problem'
      CALL XERRWD3 (MSG, 50, 102, 1, 1, MXHNIL, 0, 0, ZERO, ZERO)
 290  CONTINUE
C-----------------------------------------------------------------------
C CALL ZVSTEP1 (Y, YH, NYH, YH, EWT, SAVF, VSAV, ACOR,
C              WM, IWM, F, JAC, F, ZVNLSD1, RPAR, IPAR)
C-----------------------------------------------------------------------
c!$OMP CRITICAL
      CALL ZVSTEP1 (Y, ZWORK(LYH), NYH, ZWORK(LYH), RWORK(LEWT),
     1   ZWORK(LSAVF), Y, ZWORK(LACOR), ZWORK(LWM), IWORK(LIWM),
     2   F, JAC, F, ZVNLSD1, RPAR, IPAR)
      KGO = 1 - KFLAG
c!$OMP END CRITICAL
c!$OMP BARRIER
...
C-----------------------------------------------------------------------
C Block G.
C The following block handles all successful returns from ZVODE1.
...
C The maximum number of steps was taken before reaching TOUT. ----------
 500  MSG = 'ZVODE1--  At current T (=R1), MXSTEP (=I1) steps   '
      CALL XERRWD3 (MSG, 50, 201, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG = '      taken on this call before reaching TOUT     '
      CALL XERRWD3 (MSG, 50, 201, 1, 1, MXSTEP, 0, 1, TN, ZERO)
      ISTATE = -1
      GO TO 580
C EWT(i) .le. 0.0 for some i (not at start of problem). ----------------
 510  EWTI = RWORK(LEWT+I-1)
      MSG = 'ZVODE1--  At T (=R1), EWT(I1) has become R2 .le. 0.'
      CALL XERRWD3 (MSG, 50, 202, 1, 1, I, 0, 2, TN, EWTI)
      ISTATE = -6
      GO TO 580
C Too much accuracy requested for machine precision. -------------------
 520  MSG = 'ZVODE1--  At T (=R1), too much accuracy requested  '
      CALL XERRWD3 (MSG, 50, 203, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG = '      for precision of machine:   see TOLSF (=R2) '
      CALL XERRWD3 (MSG, 50, 203, 1, 0, 0, 0, 2, TN, TOLSF)
      RWORK(14) = TOLSF
      ISTATE = -2
      GO TO 580
C KFLAG = -1.  Error test failed repeatedly or with ABS(H) = HMIN. -----
 530  MSG = 'ZVODE1--  At T(=R1) and step size H(=R2), the error'
      CALL XERRWD3 (MSG, 50, 204, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG = '      test failed repeatedly or with abs(H) = HMIN'
      CALL XERRWD3 (MSG, 50, 204, 1, 0, 0, 0, 2, TN, H)
      ISTATE = -4
      GO TO 560
C KFLAG = -2.  Convergence failed repeatedly or with ABS(H) = HMIN. ----
 540  MSG = 'ZVODE1--  At T (=R1) and step size H (=R2), the    '
      CALL XERRWD3 (MSG, 50, 205, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG = '      corrector convergence failed repeatedly     '
      CALL XERRWD3 (MSG, 50, 205, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG = '      or with abs(H) = HMIN   '
      CALL XERRWD3 (MSG, 30, 205, 1, 0, 0, 0, 2, TN, H)
      ISTATE = -5
...
C-----------------------------------------------------------------------
 601  MSG = 'ZVODE1--  ISTATE (=I1) illegal '
      CALL XERRWD3 (MSG, 30, 1, 1, 1, ISTATE, 0, 0, ZERO, ZERO)
      IF (ISTATE .LT. 0) GO TO 800
      GO TO 700
 602  MSG = 'ZVODE1--  ITASK (=I1) illegal  '
      CALL XERRWD3 (MSG, 30, 2, 1, 1, ITASK, 0, 0, ZERO, ZERO)
      GO TO 700
 603  MSG='ZVODE1--  ISTATE (=I1) .gt. 1 but ZVODE1 not initialized  '
      CALL XERRWD3 (MSG, 60, 3, 1, 1, ISTATE, 0, 0, ZERO, ZERO)
      GO TO 700
 604  MSG = 'ZVODE1--  NEQ (=I1) .lt. 1     '
      CALL XERRWD3 (MSG, 30, 4, 1, 1, NEQ, 0, 0, ZERO, ZERO)
      GO TO 700
 605  MSG = 'ZVODE1--  ISTATE = 3 and NEQ increased (I1 to I2)  '
      CALL XERRWD3 (MSG, 50, 5, 1, 2, N, NEQ, 0, ZERO, ZERO)
      GO TO 700
 606  MSG = 'ZVODE1--  ITOL (=I1) illegal   '
      CALL XERRWD3 (MSG, 30, 6, 1, 1, ITOL, 0, 0, ZERO, ZERO)
      GO TO 700
 607  MSG = 'ZVODE1--  IOPT (=I1) illegal   '
      CALL XERRWD3 (MSG, 30, 7, 1, 1, IOPT, 0, 0, ZERO, ZERO)
      GO TO 700
 608  MSG = 'ZVODE1--  MF (=I1) illegal     '
      CALL XERRWD3 (MSG, 30, 8, 1, 1, MF, 0, 0, ZERO, ZERO)
      GO TO 700
 609  MSG = 'ZVODE1--  ML (=I1) illegal:  .lt.0 or .ge.NEQ (=I2)'
      CALL XERRWD3 (MSG, 50, 9, 1, 2, ML, NEQ, 0, ZERO, ZERO)
      GO TO 700
 610  MSG = 'ZVODE1--  MU (=I1) illegal:  .lt.0 or .ge.NEQ (=I2)'
      CALL XERRWD3 (MSG, 50, 10, 1, 2, MU, NEQ, 0, ZERO, ZERO)
      GO TO 700
 611  MSG = 'ZVODE1--  MAXORD (=I1) .lt. 0  '
      CALL XERRWD3 (MSG, 30, 11, 1, 1, MAXORD, 0, 0, ZERO, ZERO)
      GO TO 700
 612  MSG = 'ZVODE1--  MXSTEP (=I1) .lt. 0  '
      CALL XERRWD3 (MSG, 30, 12, 1, 1, MXSTEP, 0, 0, ZERO, ZERO)
      GO TO 700
 613  MSG = 'ZVODE1--  MXHNIL (=I1) .lt. 0  '
      CALL XERRWD3 (MSG, 30, 13, 1, 1, MXHNIL, 0, 0, ZERO, ZERO)
      GO TO 700
 614  MSG = 'ZVODE1--  TOUT (=R1) behind T (=R2)      '
      CALL XERRWD3 (MSG, 40, 14, 1, 0, 0, 0, 2, TOUT, T)
      MSG = '      integration direction is given by H0 (=R1)  '
      CALL XERRWD3 (MSG, 50, 14, 1, 0, 0, 0, 1, H0, ZERO)
      GO TO 700
 615  MSG = 'ZVODE1--  HMAX (=R1) .lt. 0.0  '
      CALL XERRWD3 (MSG, 30, 15, 1, 0, 0, 0, 1, HMAX, ZERO)
      GO TO 700
 616  MSG = 'ZVODE1--  HMIN (=R1) .lt. 0.0  '
      CALL XERRWD3 (MSG, 30, 16, 1, 0, 0, 0, 1, HMIN, ZERO)
      GO TO 700
 617  CONTINUE
      MSG='ZVODE1--RWORK length needed, LENRW (=I1), exceeds LRW (=I2)'
      CALL XERRWD3 (MSG, 60, 17, 1, 2, LENRW, LRW, 0, ZERO, ZERO)
      GO TO 700
 618  CONTINUE
      MSG='ZVODE1-- IWORK length needed, LENIW (=I1), exceeds LIW (=I2)'
      CALL XERRWD3 (MSG, 60, 18, 1, 2, LENIW, LIW, 0, ZERO, ZERO)
      GO TO 700
 619  MSG = 'ZVODE1--  RTOL(I1) is R1 .lt. 0.0        '
      CALL XERRWD3 (MSG, 40, 19, 1, 1, I, 0, 1, RTOLI, ZERO)
      GO TO 700
 620  MSG = 'ZVODE1--  ATOL(I1) is R1 .lt. 0.0        '
      CALL XERRWD3 (MSG, 40, 20, 1, 1, I, 0, 1, ATOLI, ZERO)
      GO TO 700
 621  EWTI = RWORK(LEWT+I-1)
      MSG = 'ZVODE1--  EWT(I1) is R1 .le. 0.0         '
      CALL XERRWD3 (MSG, 40, 21, 1, 1, I, 0, 1, EWTI, ZERO)
      GO TO 700
 622  CONTINUE
      MSG='ZVODE1-- TOUT (=R1) too close to T(=R2) to start integration'
      CALL XERRWD3 (MSG, 60, 22, 1, 0, 0, 0, 2, TOUT, T)
      GO TO 700
 623  CONTINUE
      MSG='ZVODE1--  ITASK = I1 and TOUT (=R1) behind TCUR - HU (= R2) '
      CALL XERRWD3 (MSG, 60, 23, 1, 1, ITASK, 0, 2, TOUT, TP)
      GO TO 700
 624  CONTINUE
      MSG='ZVODE1--  ITASK = 4 or 5 and TCRIT (=R1) behind TCUR (=R2)  '
      CALL XERRWD3 (MSG, 60, 24, 1, 0, 0, 0, 2, TCRIT, TN)
      GO TO 700
 625  CONTINUE
      MSG='ZVODE1--  ITASK = 4 or 5 and TCRIT (=R1) behind TOUT (=R2)  '
      CALL XERRWD3 (MSG, 60, 25, 1, 0, 0, 0, 2, TCRIT, TOUT)
      GO TO 700
 626  MSG = 'ZVODE1--  At start of problem, too much accuracy   '
      CALL XERRWD3 (MSG, 50, 26, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG='      requested for precision of machine:   see TOLSF (=R1) '
      CALL XERRWD3 (MSG, 60, 26, 1, 0, 0, 0, 1, TOLSF, ZERO)
      RWORK(14) = TOLSF
      GO TO 700
 627  MSG='ZVODE1--  Trouble from ZVINDY1.  ITASK = I1, TOUT = R1.     '
      CALL XERRWD3 (MSG, 60, 27, 1, 1, ITASK, 0, 1, TOUT, ZERO)
      GO TO 700
 628  CONTINUE
      MSG='ZVODE1-- ZWORK length needed, LENZW (=I1), exceeds LZW (=I2)'
      CALL XERRWD3 (MSG, 60, 17, 1, 2, LENZW, LZW, 0, ZERO, ZERO)
C
 700  CONTINUE
      ISTATE = -3
      RETURN
C
 800  MSG = 'ZVODE1--  Run aborted:  apparent infinite loop     '
      CALL XERRWD3 (MSG, 50, 303, 2, 0, 0, 0, 0, ZERO, ZERO)
      RETURN
C----------------------- End of Subroutine ZVODE1 -----------------------
      END SUBROUTINE ZVODE1
      ...
      SUBROUTINE ZVINDY1 (T, K, YH, LDYH, DKY, IFLAG)
...
C     /ZVOD011/ --  H, TN, UROUND, L, N, NQ
C     /ZVOD021/ --  HU
C
C Subroutines called by ZVINDY1: DZSCAL, XERRWD3
C Function routines called by ZVINDY1: None
C-----------------------------------------------------------------------
C ZVINDY1 computes interpolated values of the K-th derivative of the
...
C
 ...
 55   R = H**(-K)
      ...
      RETURN
C
 80   MSG = 'ZVINDY1-- K (=I1) illegal      '
      CALL XERRWD3 (MSG, 30, 51, 1, 1, K, 0, 0, ZERO, ZERO)
      IFLAG = -1
      RETURN
 90   MSG = 'ZVINDY1-- T (=R1) illegal      '
      CALL XERRWD3 (MSG, 30, 52, 1, 0, 0, 0, 1, T, ZERO)
      MSG='      T not in interval TCUR - HU (= R1) to TCUR (=R2)      '
      CALL XERRWD3 (MSG, 60, 52, 1, 0, 0, 0, 2, TP, TN)
      IFLAG = -2
      RETURN
C----------------------- End of Subroutine ZVINDY1 ----------------------
      END SUBROUTINE ZVINDY1
      ...
      SUBROUTINE ZVSTEP1 (Y, YH, LDYH, YH1, EWT, SAVF, VSAV, ACOR,
     1                  WM, IWM, F, JAC, PSOL, VNLS, RPAR, IPAR)
      EXTERNAL F, JAC, PSOL, VNLS
      DOUBLE COMPLEX Y, YH, YH1, SAVF, VSAV, ACOR, WM, RPAR
...
C COMMON block variables accessed:
C     /ZVOD011/  ACNRM, EL(13), H, HMIN, HMXI, HNEW, HSCAL, RC, TAU(13),
C               TQ(5), TN, JCUR, JSTART, KFLAG, KUTH,
C               L, LMAX, MAXORD, N, NEWQ, NQ, NQWAIT
C     /ZVOD021/  HU, NCFN, NETF, NFE, NQU, NST
C
C Subroutines called by ZVSTEP1: F, DZAXPY, ZCOPY, DZSCAL,
C                               ZVJUST1, VNLS, ZVSET1
C Function routines called by ZVSTEP1: ZVNORM
C-----------------------------------------------------------------------
C ZVSTEP1 performs one step of the integration of an initial value
C problem for a system of ordinary differential equations.
C ZVSTEP1 calls subroutine VNLS for the solution of the nonlinear system
...
C ZVSTEP1 returns a completion flag KFLAG (in COMMON).
...
C
C Communication with ZVSTEP1 is done with the following variables:
C
...
!$OMP THREADPRIVATE(ADDON,BIAS1,BIAS2,BIAS3,ETACF,ETAMIN,ETAMX1,ETAMX2,
!$OMP& ETAMX3,ETAMXF,ETAQ,ETAQM1,KFC,KFH,MXNCF,ONEPSM,THRESH,ONE,ZERO)
C-----------------------------------------------------------------------
      COMMON /ZVOD011/ ACNRM, CCMXJ, CONP, CRATE, DRC, EL(13), ETA,
     1                ETAMAX, H, HMIN, HMXI, HNEW, HRL1, HSCAL, PRL1,
     2                RC, RL1, SRUR, TAU(13), TQ(5), TN, UROUND,
...
C On an order decrease, the history array is adjusted by ZVJUST1.
...
C ZVSET1 is called to calculate all integration coefficients.
...
C-----------------------------------------------------------------------
 ...
 720  JSTART = 1
      RETURN
C----------------------- End of Subroutine ZVSTEP1 ----------------------
      END SUBROUTINE ZVSTEP1
...
C COMMON block variables accessed:
C     /ZVOD011/ -- EL(13), H, TAU(13), TQ(5), L(= NQ + 1),
C                 METH, NQ, NQWAIT
C
C Subroutines called by ZVSET1: None
C Function routines called by ZVSET1: None
C-----------------------------------------------------------------------
C ZVSET1 is called by ZVSTEP1 and sets coefficients for use there.
C
...
C with ZVSET1 uses the following:
...
C In EL, construct coefficients of (1+x/xi(1))*...*(1+x/xi(j+1)). ------
 ...
 300  TQ(4) = CORTES*TQ(2)
      RETURN
C----------------------- End of Subroutine ZVSET1 -----------------------
      END SUBROUTINE ZVSET1
      ...
      SUBROUTINE ZVJUST1 (YH, LDYH, IORD)
      DOUBLE COMPLEX YH
      INTEGER LDYH, IORD
      DIMENSION YH(LDYH,*)
C-----------------------------------------------------------------------
...
C COMMON block variables accessed:
C     /ZVOD011/ -- HSCAL, TAU(13), LMAX, METH, N, NQ,
C
C Subroutines called by ZVJUST1: DZAXPY
C Function routines called by ZVJUST1: None
C-----------------------------------------------------------------------
...
C Communication with ZVJUST1 uses the following:
...
C         (If IORD = +1, ZVJUST1 assumes that HSCAL = TAU(1).)
...
C Add correction terms to YH array. ------------------------------------
 ...
 370  CONTINUE
      RETURN
C----------------------- End of Subroutine ZVJUST1 ----------------------
      END SUBROUTINE ZVJUST1
      ...
      SUBROUTINE ZVNLSD1 (Y, YH, LDYH, VSAV, SAVF, EWT, ACOR, IWM, WM,
     1                 F, JAC, PDUM, NFLAG, RPAR, IPAR)
      EXTERNAL F, JAC, PDUM
      DOUBLE COMPLEX Y, YH, VSAV, SAVF, ACOR, WM, RPAR
      DOUBLE PRECISION EWT
...
C COMMON block variables accessed:
C     /ZVOD011/ ACNRM, CRATE, DRC, H, RC, RL1, TQ(5), TN, ICF,
C                JCUR, METH, MITER, N, NSLP
C     /ZVOD021/ HU, NCFN, NETF, NFE, NJE, NLU, NNI, NQU, NST
C
C Subroutines called by ZVNLSD1: F, DZAXPY, ZCOPY, DZSCAL, ZVJAC1, ZVSOL1
C Function routines called by ZVNLSD1: ZVNORM
C-----------------------------------------------------------------------
C Subroutine ZVNLSD1 is a nonlinear system solver, which uses functional
C iteration or a chord (modified Newton) method.  For the chord method
C direct linear algebraic system solvers are used.  Subroutine ZVNLSD1
C then handles the corrector phase of this integration package.
C
C Communication with ZVNLSD1 is done with the following variables. (For
...
C                 -1 convergence failure in previous call to ZVNLSD1.
C                 -2 error test failure in ZVSTEP1.
C              OUTPUT
...
C to force ZVJAC1 to be called, if a Jacobian is involved.
C In any case, ZVJAC1 is called at least every MSBP steps.
C-----------------------------------------------------------------------
      DRC = ABS(RC-ONE)
...
C-----------------------------------------------------------------------
      CALL ZVJAC1 (Y, YH, LDYH, EWT, ACOR, SAVF, WM, IWM, F, JAC, IERPJ,
     1           RPAR, IPAR)
      IPUP = 0
      RC = ONE
      DRC = ZERO
...
C 2/(1+RC) to account for changes in h*rl1 since the last ZVJAC1 call.
C-----------------------------------------------------------------------
 350  DO 360 I = 1,N
 360    Y(I) = (RL1*H)*SAVF(I) - (RL1*YH(I,2) + ACOR(I))
      CALL ZVSOL1 (WM, IWM, Y, IERSL)
      NNI = NNI + 1
...
C Return for successful step. ------------------------------------------
 450  NFLAG = 0
      ...
      RETURN
C----------------------- End of Subroutine ZVNLSD1 ----------------------
      END SUBROUTINE ZVNLSD1
      ...
      SUBROUTINE ZVJAC1 (Y, YH, LDYH, EWT, FTEM, SAVF, WM, IWM, F, JAC,
     1                 IERPJ, RPAR, IPAR)
      EXTERNAL F, JAC
      DOUBLE COMPLEX Y, YH, FTEM, SAVF, WM, RPAR
      DOUBLE PRECISION EWT
...
C COMMON block variables accessed:
C     /ZVOD011/  CCMXJ, DRC, H, HRL1, RL1, SRUR, TN, UROUND, ICF, JCUR,
C               LOCJS, MITER, MSBJ, N, NSLJ
C     /ZVOD021/  NFE, NST, NJE, NLU
C
C Subroutines called by ZVJAC1: F, JAC, ZACOPY, ZCOPY, ZGBFA, ZGEFA,
C                              DZSCAL
C Function routines called by ZVJAC1: ZVNORM
C-----------------------------------------------------------------------
C ZVJAC1 is called by ZVNLSD1 to compute and process the matrix
...
C
C Communication with ZVJAC1 is done with the following variables.  (For
C more details, please see the comments in the driver subroutine.)
...
C
C----------------------- End of Subroutine ZVJAC1 -----------------------
      END SUBROUTINE ZVJAC1
      ...
      SUBROUTINE ZVSOL1 (WM, IWM, X, IERSL)
      DOUBLE COMPLEX WM, X
      INTEGER IWM, IERSL
      DIMENSION WM(*), IWM(*), X(*)
C-----------------------------------------------------------------------
...
C COMMON block variables accessed:
C     /ZVOD011/ -- H, HRL1, RL1, MITER, N
C
C Subroutines called by ZVSOL1: ZGESL, ZGBSL
C Function routines called by ZVSOL1: None
C-----------------------------------------------------------------------
...
C Communication with ZVSOL1 uses the following variables:
...
C
 400  ML = IWM(1)
      ...
      RETURN
C----------------------- End of Subroutine ZVSOL1 -----------------------
      END SUBROUTINE ZVSOL1
      ...
      SUBROUTINE ZVSRCO1 (RSAV, ISAV, JOB)
...
C COMMON block variables accessed -- All of /ZVOD011/ and /ZVOD021/
C
C Subroutines/functions called by ZVSRCO1: None
C-----------------------------------------------------------------------
C This routine saves or restores (depending on JOB) the contents of the
C COMMON blocks ZVOD011 and ZVOD021, which are used internally by ZVODE1.
C
...
C
      COMMON /ZVOD011/ RVOD1(50), IVOD1(33)
      COMMON /ZVOD021/ RVOD2(1), IVOD2(8)
...
C
      RETURN
C----------------------- End of Subroutine ZVSRCO1 ----------------------
      END SUBROUTINE ZVSRCO1

      END MODULE zvode1_mod
